      ...
      let rec inner_loop curr_col s_coords e_coords =
        if Int.equal curr_col max_col then loop (curr_row + 1) s_coords e_coords
          ...
          | "S" | "a" ->
              ...
              inner_loop (curr_col + 1)
                ((curr_row, curr_col) :: s_coords)
                e_coords
          | "E" ->
...
let bfs grid start_coords ~end_coords =
  ...
  let rec loop () =
    if Pairing_heap.is_empty queue then ()
      ...
      if equal_coords curr end_coords then ()
      else
  ...
  in
  loop ();
  Hashtbl.find dist_map end_coords

let lines = In_channel.read_lines "../lib/p12.txt"
let tmp = grid lines
let lowest_elevation_list, (end_row, end_col) = search_start_goal tmp
(* let () = List.iter ~f:print_point lowest_elevation_list *)
...
let x =
  List.filter_map
    ~f:(bfs config ~end_coords:(end_row, end_col))
    lowest_elevation_list
  |> List.fold ~init:Int.max_value ~f:(fun accum e -> min accum @@ e)
