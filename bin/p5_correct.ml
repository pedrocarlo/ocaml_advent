...
let parse_config s =
  ...
  |> List.drop_last_exn |> List.to_array
  |> Array.transpose_exn (* Transpose array so it naturally becomes the stack *)
  |> Array.filteri ~f:(fun i _ -> i mod 4 = 1)
     (* Only selecting chars here skipping [] adn intermediate whitespace *)
  |> Array.map ~f:Array.to_list
  |> Array.map ~f:(fun l -> List.drop_while l ~f:Char.is_whitespace)
(* dropping the whitespace elements of stack as they wont be used*)
...
let _part2_place to_place dst = to_place @ dst

let _ =
  let input = In_channel.read_all "../lib/p5.txt" in
  let config, instructions = parse_input input in
  execute config instructions _part1_place;
